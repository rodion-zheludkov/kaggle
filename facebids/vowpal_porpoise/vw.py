...
from vowpal_porpoise.vp_utils import safe_remove, VPLogger
...
class VW:
    ...
    def vw_base_command(self, base, test=False):
        l = base
        ...
        if not test and self.bfgs:               l.append('--bfgs')
        if self.adaptive:                        l.append('--adaptive')
        if self.nn                  is not None: l.append('--nn=%d' % self.nn)
        if self.holdoff:
            l.append('--holdout_off')
        # l.append('--early_terminate=10')
    ...
    def vw_test_command(self, model_file, prediction_file):
        if self.oaa:
            return self.vw_base_command([self.vw], True) + ' -t -i %s -r %s' % (model_file, prediction_file)
        else:
            return self.vw_base_command([self.vw], True) + ' -t -i %s -p %s' % (model_file, prediction_file)
    ...
    def start_predicting_library(self):
        model_file = self.get_model_file()
        self.vw_process = VW(self.vw_test_command_library(model_file))
